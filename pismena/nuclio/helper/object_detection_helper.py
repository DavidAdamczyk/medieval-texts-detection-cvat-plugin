...
def nms(boxes, scores, context, thresh=0.5):
    #context.logger.info(f"starting nms bboxes: {len(boxes)}")
    ...
    while len(scores) > 0:
        ...
        #context.logger.info(f"to_keep: {to_keep}")
        iou_vals = IoU_values(boxes, boxes[:1]).squeeze()
        #context.logger.info(f"boxes: {boxes}")
        mask_keep = iou_vals <= thresh
        #context.logger.info(f"iou_vals: {iou_vals}")
        #context.logger.info(f"mask_keep: {mask_keep}")
        if len(torch.nonzero(mask_keep, as_tuple=False)) == 0: break
        boxes, scores, indexes = boxes[mask_keep], scores[mask_keep], indexes[mask_keep]
        #context.logger.info(f"len scores: {len(scores)}")
        if i > 10:
            break
